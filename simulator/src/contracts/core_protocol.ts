...
// only boost during normal operations
    ...
    let current_boost: bigint = this.this.boost_pool.get(boostee)! || BigInt("0");
    let new_boost: bigint = current_boost + amount;
    this.boost_pool.set(boostee, new_boost);
    
    let current_total_boost: bigint = this.this.boost_pool.get(this.CORE_PROTOCOL)! || BigInt("0");
    let new_total_boost: bigint = current_total_boost + amount;
    
    this.boost_pool.set(this.CORE_PROTOCOL, new_boost);
    
    let current_total_protocol_balance: bigint = this.this.core_protocol_balance.get(BigInt("0"))! || BigInt("0");
...
// clearing the boost pool may only happen when the protocol is waiting to claim its unbonded stakes
    ...
    let boosted_amount: bigint = this.this.boost_pool.get(boostee)! || BigInt("0");
...
// clearing the boost pool may only happen when the protocol is waiting to claim its unbonded stakes
    ...
    let new_total_boost: bigint = this.this.boost_pool.get(this.CORE_PROTOCOL)! || BigInt("0");
...
// ********** CALCULATE IF IT IS PROFITABLE TO REBALANCE **********
    ...
    let last_period_duration: bigint = (this.block.height - this.this.last_rebalance_height.get(BigInt("0"))! || BigInt("0"));
...
// compound once
    ...
    let projected_stake_after_rebalance: bigint = current_stake + this.this.core_protocol_balance.get(BigInt("0"))! || BigInt("0") + rewards_from_last_period;
...
// ********** ASSERT MINT AMOUNTS **********
    ...
    let protocol_pending_withdraw_balance: bigint = this.this.core_protocol_balance.get(BigInt("1"))! || BigInt("0");
    let pool_balance: bigint = this.this.core_protocol_balance.get(BigInt("0"))! || BigInt("0");
...
// make sure the boost pool has been cleared
    let boost_pool_total: bigint = this.this.boost_pool.get(this.CORE_PROTOCOL)! || BigInt("0");
    ...
    let protocol_balance: bigint = this.this.core_protocol_balance.get(BigInt("0"))! || BigInt("0");
...
// burn full amount of ale, but immediately mint the fee to the protocol. Only withdraw credits_to_claim as the difference
  ...
  finalize_create_withdraw_claim(
    ...
    let total_withdraw_amount: bigint = this.this.withdraw_pool.get(withdraw_block)! || BigInt("0");
...
// aleo_before * ale_after = aleo_after * ale_before
  ...
  finalize_withdraw_unbond(
    ...
    let d4_state: state = this.delegator_state.get(this.DELEGATOR_4)!;
    let d5_state: state = this.delegator_state.get(this.DELEGATOR_5)!;
    let saved_withdraw_amount: bigint = this.this.withdraw_pool.get(withdraw_block)! || BigInt("0");
...
// *********** UPDATE PROTOCOL STATE **********
    ...
    let withdraw_pool_total: bigint = this.this.core_protocol_balance.get(BigInt("1"))! || BigInt("0");
...
// transfer aleo to pool
  ...
  finalize_deposit_private(
    ...
    let pending_withdraw_balance: bigint = this.this.core_protocol_balance.get(BigInt("1"))! || BigInt("0");
