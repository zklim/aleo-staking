...
// interfaces
export class delegator4Program {
...
// this delegator should be bonded to
  ...
  //program delegator4.aleo {// 0u8 -> 0u8 = unbonding unavailable, 1u8 = unbonding available
...
// unbonding no longer available
  ...
  claim_unbond(
    transition claim_unbond(: undefined,
  ) {
    assert(this.caller === this.CORE_PROTOCOL);
    this.credits.caller = "contract";
  ...
  finalize_claim_unbond(
    finalize claim_unbond(: undefined,
  ) {
